...
document
	.getElementById('add-characteristic-btn')
...
document
	...
	.addEventListener('click', function () {
		...
		item.innerHTML = `
          ...
          <label>Опис:</label><br>
          <textarea class="editAccordion-description__textarea" name="accordion-description[]" placeholder="Опис акордеону"></textarea>
        `
	...
	.addEventListener('submit', async function (e) {
		...
		try {
			...
			const descriptions = document.querySelectorAll(
				'input[name="characteristic-description[]"]'
			)
			...
			const titles = document.querySelectorAll(
				'input[name="accordion-title[]"]'
			)
			const accordionDescriptions = document.querySelectorAll(
				'textarea[name="accordion-description[]"]'
			)
			...
			if (response.ok) {
				...
				alert(
					'Помилка: ' + (errData.message || 'Не вдалося додати інформацію.')
				)
			}
		} catch (error) {
